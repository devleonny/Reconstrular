...
async function abrirEPI(idColaborador) {
    ...
    try {
        ...
        const { nome, epi } = await recuperarDado('dados_colaboradores', idColaborador) || {}
        const { equipamentos, data } = epi || {}
        const tdG = (el) => `<td style="text-align: center;">${el || ''}</div>`

        const trEquipamentos = Object.entries(equipamentos || {})
            .map(([nomeEquip, { quantidade, tamanho }]) => {

                const { risco, nome } = equipamentosLinhas?.[nomeEquip] || {}
                ...
                <tr>
                    ...
                    ${tdG()}
                    ${tdG()}
                    ${tdG(data ? new Date(data).toLocaleString() : '')}
                </tr>
            `
            })
            .join('')
        ...
        <div class="painelFiltros">
            <img onclick="baixarFichaEPI('${nome}')" src="imagens/pdf.png">
            <button onclick="telaColaboradores()">Voltar</button>
        </div>

        <div id="pdf" class="contornoFolha">
            <div class="folha-epi">
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            <td><strong>FUNCIONÁRIO: ${nome || ''}</strong></td>
                        </tr>
    ...
    `
        ...
        console.error(err)
        popup({ mensagem: 'Falha ao abrir o PDF do EPI: Fale com o suporte.' })
    }
}

async function baixarFichaEPI(nomeColaborador) {
    ...
    try {
        ...
        console.error(err)
        popup({ mensagem: 'Falha ao baixar FICHA DE EPI: Fale com o suporte.' })
    }
